...
export const useViewStore = create<ViewState>((set, get) => ({
  ...
  setView: (v) => {
    ...
    // Scroll to top invisibly BEFORE capturing the old snapshot,
    // so the view transition shows both views at scroll position 0
    // without any visible scroll-bar movement during the crossfade.
    if (typeof window !== "undefined") {
      window.scrollTo({ top: 0, behavior: "instant" });
    }

    // Wrap the update in a View Transition for smooth crossfade between views.
    // The browser captures the old state before the callback, runs the DOM
    // mutation synchronously (via flushSync), then captures the new state and
    // interpolates between them.
    // Falls back to instant update in unsupported browsers.
    if (
