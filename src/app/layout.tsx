...
// ── Font strategy ─────────────────────────────────────────────
//
// Cairo (body/sans):   display="optional" — The body font uses a very short
//   block period (~100ms). If the font arrives within that window, it renders
//   immediately. If not, the system fallback is used for the current page
//   load. On repeat visits the font is cached so it appears instantly.
//   This completely eliminates the Flash of Unstyled Text (FOUT) that
//   "swap" causes on slow connections, which is especially jarring for
//   body text across font-size/weight variants within the same paragraph.
//
// Amiri (display/naskh): display="swap" — The decorative heading font is
//   worth waiting for. A brief FOUT is acceptable here since headings are
//   visually distinct and the swap is less noticeable.
